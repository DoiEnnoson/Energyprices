"""
generate_newsletter.py – HTML-E-Mail mit eingebetteten Charts
"""
...
def b64img(filename: str) -> str:
    """PNG aus output/ als data-URI einbetten – absoluter Pfad"""
    p = ROOT / C.OUTPUT_DIR / filename
    if not p.exists():
        log.warning(f"Chart nicht gefunden: {p}")
        return ""
...
def img(filename: str, alt: str) -> str:
    src = b64img(filename)
    if not src:
        return f'<p style="color:#8b949e;font-style:italic;">[{alt} nicht verfügbar]</p>'
...
def build_html(data: dict) -> str:
    ...
    for label, key, color in [
        ("Super E5",  "e5",     C.COLORS["ice"]),
        ("E10",       "e10",    "#f59e0b"),
        ("Diesel",    "diesel", "#6b7280"),
    ]:
        if fuel.get(key):
            fuel_rows += row(f"{label} (Bundesdurchschnitt)", fuel[key], "EUR/L", color)

    # ── Fahrzeugvergleich ──────────────────────────────────────────
    vc_rows = ""
    for key, color in [
        ("ice",           C.COLORS["ice"]),
        ("bev_home",      C.COLORS["bev_home"]),
        ("bev_public_ac", C.COLORS["bev_ac"]),
        ("bev_public_dc", C.COLORS["bev_dc"]),
    ]:
        e = vc.get(key)
        if not e:
            continue
        ...
        vc_rows += f"""<tr>
          <td style="padding:7px 12px;color:#e6edf3;">{e.get('label','')}</td>
          <td style="padding:7px 12px;text-align:right;color:{color};font-weight:600;">
            {fmt(e.get('cost_per_100km'), 2, '€/100 km')} {save_str}
          </td>
          <td style="padding:7px 12px;text-align:right;color:{color};">
            {fmt(e.get('km_for_budget'), 0, 'km')} für {C.COMPARISON_BUDGET_EUR:.0f} €
          </td></tr>"""

    # ── Heizkosten ─────────────────────────────────────────────────
    heat_rows = ""
    color_map = {
        "gas_boiler": C.COLORS["gas"], "oil_boiler": C.COLORS["oil_heat"],
    ...
    for prop_key, prop_label in [
        ...
        heat_rows += f"""<tr style="background:#1c2128;">
          <td colspan="2" style="padding:9px 12px;color:#e6edf3;font-weight:700;
             border-top:1px solid #21262d;">{prop_label}</td></tr>"""
        for sk, slabel in [
            ("gas_boiler","Gasheizung"), ("oil_boiler","Ölheizung"),
            ("heat_pump","Wärmepumpe (COP 3,5)"), ("direct_electric","Direktstrom"),
        ]:
            cost = heat.get(prop_key,{}).get("systems",{}).get(sk,{}).get("weekly_cost_eur")
            color = color_map.get(sk, "#e6edf3")
            heat_rows += f"""<tr>
              <td style="padding:6px 12px 6px 24px;color:#e6edf3;">{slabel}</td>
              <td style="padding:6px 12px;text-align:right;color:{color};font-weight:600;">
                {fmt(cost, 2, '€/Woche')}
              </td></tr>"""
...
<table width="680" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="background:linear-gradient(135deg,#1c2128,#0d1117);padding:28px 32px;">
    <p style="margin:0 0 4px;font-size:12px;color:#3b82f6;letter-spacing:1.5px;
      text-transform:uppercase;">Täglicher Energie-Report</p>
    <h1 style="margin:0 0 6px;font-size:24px;font-weight:700;color:#e6edf3;">
      Energiepreise Deutschland</h1>
    <p style="margin:0;font-size:13px;color:#8b949e;">
      Stand {today} · Tagesdaten ab 1. Januar 2026 · Index 100 = 1.1.2026</p>
  </td></tr>
  ...
  <tr><td style="padding:24px 32px 8px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
      ...
      <thead><tr style="background:#1c2128;">
        <th style="padding:9px 12px;text-align:left;color:#8b949e;font-size:11px;
          text-transform:uppercase;">Energieträger</th>
        <th style="padding:9px 12px;text-align:right;color:#8b949e;font-size:11px;
          text-transform:uppercase;">Aktuell</th>
      </tr></thead>
      <tbody>{market_rows}{fuel_rows}</tbody>
    </table>
  </td></tr>
  ...
  <tr><td style="padding:8px 32px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
      ...
      <thead><tr style="background:#1c2128;">
        <th style="padding:9px 12px;text-align:left;color:#8b949e;font-size:11px;
          text-transform:uppercase;">Antrieb</th>
        <th style="padding:9px 12px;text-align:right;color:#8b949e;font-size:11px;
          text-transform:uppercase;">Kosten/100 km</th>
        <th style="padding:9px 12px;text-align:right;color:#8b949e;font-size:11px;
          text-transform:uppercase;">Reichweite</th>
      </tr></thead>
      <tbody>{vc_rows}</tbody>
    </table>
  ...
  <tr><td style="padding:8px 32px 16px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
      ...
      <thead><tr style="background:#1c2128;">
        ...
        <th style="padding:9px 12px;text-align:right;color:#8b949e;font-size:11px;
          text-transform:uppercase;">€/Woche</th>
      </tr></thead>
  ...
  <tr><td style="padding:12px 32px 24px;border-top:1px solid #21262d;">
    <p style="margin:0;font-size:11px;color:#8b949e;line-height:1.7;">
      ...
      Energy-Charts/ENTSO-E · Yahoo Finance (BZ=F, TTF=F, HO=F) ·
      Tankerkönig/MTS-K · BDEW ({ref.get('bdew_reference_period','')}) ·
      GlobalPetrolPrices.com<br>
      Haushaltsstrom {ref.get('bdew_electricity_ct_kwh','')} ct/kWh ·
      Haushaltsgas {ref.get('bdew_gas_ct_kwh','')} ct/kWh ·
      Öffentliches Laden AC {ref.get('public_charging_ac_ct_kwh','')} ct/kWh ·
      DC {ref.get('public_charging_dc_ct_kwh','')} ct/kWh<br>
...
def main():
    ...
    # Web-JSON
    web = ROOT / C.OUTPUT_DIR / "web_data.json"
    with open(web, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    log.info(f"Web-JSON  → {web}")
